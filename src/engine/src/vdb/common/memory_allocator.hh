...
#define AllocateNoPrefix(n) \
  vdb::base_allocator.AllocateNoPrefixInternal(n, __FILE__, __LINE__)
...
#define AllocateWithPrefix(n) \
  vdb::base_allocator.AllocateWithPrefixInternal(n, __FILE__, __LINE__)
...
#define AllocateAligned(alignment, n) \
  vdb::base_allocator.AllocateAlignedInternal(alignment, n, __FILE__, __LINE__)
...
class BaseAllocator {
 public:
  ...
  /* if allocated size is managed by higher layer, don't need to allocate memory
   * with prefix(size) */
  void* AllocateNoPrefixInternal(uint64_t n, const char* file = __FILE__,
  ...
  /* length of allocated size is stored in prefix portion */
  void* AllocateWithPrefixInternal(uint64_t n, const char* file = __FILE__,
                                   const int line = __LINE__);
  ...
  /* aligned chunk is allocated. no prefix is stored. */
  void* AllocateAlignedInternal(uint64_t alignment, uint64_t n,
                                const char* file = __FILE__,
                                const int line = __LINE__);
 ...
 public:
  ...
  pointer allocate(size_type n) {
    return static_cast<pointer>(
        BaseAllocator::AllocateNoPrefixInternal(n * sizeof(T)));
  }
...
class SizeAwareAllocator : public std::allocator<T>, public BaseAllocator {
 public:
  ...
  pointer allocate(size_type n) {
    return static_cast<pointer>(
        BaseAllocator::AllocateWithPrefixInternal(n * sizeof(T)));
  }
...
class AlignedAllocator : public std::allocator<T>, public BaseAllocator {
 public:
  ...
  pointer allocate(size_type n) {
    return static_cast<pointer>(
        BaseAllocator::AllocateAlignedInternal(alignment_, n * sizeof(T)));
  }
